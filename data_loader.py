...
def load_dataset(opt,train=True):
	...
	dataset = dataset. \
	apply(tf.data.experimental.shuffle_and_repeat(dataset_num)). \
	apply(tf.data.experimental.map_and_batch(_parse_function, opt.batch_size, num_parallel_batches=4, drop_remainder=True)). \
	apply(tf.data.experimental.prefetch_to_device('/gpu:0', None)) # When using dataset.prefetch, use buffer_size=None to let it detect optimal buffer size
