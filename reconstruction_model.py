...
class Reconstruction_model():
	# initialization
	def __init__(self,opt,graph,id_latent,pca_model,D, L, F, K, p, nv,nz,U,F_0,filter_='chebyshev5', brelu='b1relu', unpool='poolwT',pool='poolwT',regularization=0, dropout=0):
		...
		self.D,self.L, self.F, self.K, self.p, self.nv, self.U, self.F_0 = D,L, F, K, p, nv, U, F_0
		self.filter, self.b1relu, self.unpool, self.pool,self.regularization, self.dropout = filter_, brelu, unpool, pool, regularization, dropout
	...
	def forward(self,is_train = True):
		...
		with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
			# self.coeff,self.gcn = networks.R_Net(self.imgs,is_training=is_train)
			# self.coeff = networks.R_Net(self.imgs,is_training=is_train)
			self.id_labels = networks.Perceptual_Net(self.imgs)

			# self.output_mesh = networks.GCN(self.id_latent,self.L, self.F, self.K, self.p, self.U, self.F_0)
			image_emb, self.coeff = networks.get_emb_coeff('facerec', self.imgs, self.graph)
			image_feat = networks.get_img_feat('facenet', self.imgs,self.graph)
			self.id_emb = tf.concat([image_emb, image_feat], axis=-1)
	
			self.pca_facemodel = self.Face3D.get_pca_facemodel(self.coeff,self.opt)
			
			self.output_mesh = networks.mesh_generator(self.id_latent,self.pca_model,self.D,self.L,self.F,self.K,self.p,self.U,self.F_0)

			self.Face3D.Reconstruction_Block(self.coeff,self.opt,self.output_mesh)
			...
			self.shape_loss = losses.Shape_loss(self.Face3D.face_shape_t,self.Face3D.face_shape_linear_t)
			self.normal_loss = losses.Normal_loss(self.Face3D.face_norm,self.Face3D.face_norm_linear)

			# if self.opt.pretrain_stage:
			...
			# else:
			self.loss = self.opt.w_lm*self.landmark_loss + self.opt.w_shape*self.shape_loss + self.opt.w_normal*self.normal_loss
	...
	def backward(self,is_train = True):
		if is_train:
			...
			# else:
			update_var_list = [v for v in var_list if 'gcn' in v.name]
	...
	def summarize(self):
		...
		stat = [
		tf.summary.scalar('shape_error',self.shape_loss),
		tf.summary.scalar('normal_error',self.normal_loss),
		...
		render_imgs_linear = self.Face3D.render_imgs_linear[:,:,:,::-1]*self.Face3D.img_mask_linear + tf.cast(self.imgs[:,:,:,::-1],tf.float32)*(1-self.Face3D.img_mask_linear)
		render_imgs_linear = tf.clip_by_value(render_imgs_linear,0,255)
		render_imgs_linear = tf.cast(render_imgs_linear,tf.uint8)
		# image stats
		img_stat = [tf.summary.image('imgs',tf.concat([tf.cast(self.imgs[:,:,:,::-1],tf.uint8),render_imgs_linear,render_imgs],axis = 2), max_outputs = 8)]
