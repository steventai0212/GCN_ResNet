...
def Shape_loss(gcn,linear):
	from tensorflow.python.ops import array_ops
	gcn = tf.reshape(gcn,(16,-1))
	linear = tf.reshape(linear,(16,-1))

	diff = gcn - linear
	
	shape_loss = tf.nn.l2_loss(diff)

	inputs_shape = array_ops.shape(diff)

	numel = tf.cast(tf.reduce_prod(inputs_shape), diff.dtype)
	shape_loss = tf.div(shape_loss, numel)

	shape_loss = tf.reduce_mean(shape_loss)

	return shape_loss

def Normal_loss_v1(gcn,linear):
	from tensorflow.python.ops import array_ops
	gcn = tf.reshape(gcn,(16,-1))
	linear = tf.reshape(linear,(16,-1))
	...
	normal_loss = tf.reduce_sum(tf.abs(diff))

	inputs_shape = array_ops.shape(diff)

	numel = tf.cast(tf.reduce_prod(inputs_shape), diff.dtype)
	normal_loss = tf.div(normal_loss, numel)

	normal_loss = tf.reduce_mean(normal_loss)

	return normal_loss

def Normal_loss(gcn,linear):

	gcn = tf.reshape(gcn,(16,-1))
	linear = tf.reshape(linear,(16,-1))

	normal_loss = tf.losses.mean_squared_error(gcn,linear)
