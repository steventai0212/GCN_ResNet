...
class Face3D():
	...
	def get_pca_facemodel(self,coeff,opt):
		...
		pca_facemodel = self.Shape_formation_block(id_coeff,ex_coeff,self.facemodel)
		# [batchsize,N,3] vertex texture (in RGB order)
		face_texture_linear = self.Texture_formation_block(tex_coeff,self.facemodel)
		# [batchsize,3,3] rotation matrix for face shape
		rotation = self.Compute_rotation_matrix(angles)
		# [batchsize,N,3] vertex normal
		face_norm_linear = self.Compute_norm(pca_facemodel,self.facemodel)
		norm_r_linear = tf.matmul(face_norm_linear,rotation)
		
		# do rigid transformation for face shape using predicted rotation and translation
		face_shape_linear_t = self.Rigid_transform_block(pca_facemodel,rotation,translation)
		
		# [batchsize,N,3] vertex color (in RGB order)
		face_color_linear = self.Illumination_block(face_texture_linear, norm_r_linear, gamma)
		
		render_imgs_linear,img_mask_linear,img_mask_crop_linear = self.Render_block(face_shape_linear_t,norm_r_linear,face_color_linear,camera_scale,f_scale,self.facemodel,opt.batch_size,opt.is_train)


		self.pca_facemodel         = pca_facemodel
		self.face_norm_linear      = face_norm_linear
		self.face_shape_linear_t   = face_shape_linear_t
		self.render_imgs_linear    = render_imgs_linear
		self.img_mask_linear       = img_mask_linear
		self.img_mask_crop_linear  = img_mask_crop_linear


		return pca_facemodel

	# analytic 3D face reconstructions with coefficients from R-Net
	def Reconstruction_Block(self,coeff,opt,mesh):
		#coeff: [batchsize,257] reconstruction coefficients

		id_coeff,ex_coeff,tex_coeff,angles,translation,gamma,camera_scale,f_scale = self.Split_coeff(coeff)
		# [batchsize,N,3] canonical face shape in BFM space
		# if opt.pretrain_stage:
		# 	face_shape_linear_ = mesh
		# 	face_shape = self.Shape_formation_block(id_coeff,ex_coeff,self.facemodel)
		# else:
		# 	face_shape_linear_ = self.Shape_formation_block(id_coeff,ex_coeff,self.facemodel)
		face_shape = mesh
		# [batchsize,N,3] vertex texture (in RGB order)
		...
		# face_norm_linear = self.Compute_norm(face_shape_linear_,self.facemodel)
		...
		# face_shape_linear = self.Rigid_transform_block(face_shape_linear_,rotation,translation)
		...
		self.camera_scale      = camera_scale
		self.f_scale           = f_scale
		# self.face_shape_linear = face_shape_linear
		# self.face_norm_linear  = face_norm_linear
